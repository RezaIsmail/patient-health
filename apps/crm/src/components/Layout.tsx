...
const navItems: NavItem[] = [
  ...
  { label: 'Accounts', to: '/accounts', icon: <Building2 className="h-5 w-5" /> },
  { label: 'Referrals', to: '/referrals', icon: <GitBranch className="h-5 w-5" /> },
  { label: 'Care Plans', to: '/care-plans', icon: <ClipboardList className="h-5 w-5" /> },
  { label: 'Care Gaps', to: '/care-gaps', icon: <AlertTriangle className="h-5 w-5" /> },
  { label: 'Tasks', to: '/tasks', icon: <CheckSquare className="h-5 w-5" /> },
  ...
  { label: 'Webhooks', to: '/webhooks', icon: <Webhook className="h-5 w-5" /> },
]
...
export default function Layout() {
  ...
  // Real-time bridge: EMR events → Redis → this hook → TanStack Query invalidation
  // Real-time bridge: EMR/Admin events → Redis → this hook → TanStack Query invalidation
  useServerEvents()
    ...
    <>
      <div className="flex h-14 items-center gap-2.5 border-b border-gray-100 px-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-gray-400 font-medium tracking-wide uppercase mt-0.5">CRM</p>
        </div>
        <button
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-14 items-center gap-3 border-b border-gray-200 bg-white px-4 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-medium text-gray-400 tracking-wide uppercase">CRM</span>
          </div>
          <div className="ml-auto flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-700">
            {userInitials}
          </div>
        </header>
