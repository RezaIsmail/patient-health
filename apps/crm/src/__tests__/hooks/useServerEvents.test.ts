...
describe('useServerEvents (CRM)', () => {
  ...
  it('invalidates contacts on emr:patient:created', async () => {
  ...
  it('invalidates contacts and care-gaps and dashboard on emr:clinical:synced', async () => {
    ...
    expect(mockInvalidateQueries).toHaveBeenCalledWith({ queryKey: ['care-gaps'] })
    expect(mockInvalidateQueries).toHaveBeenCalledWith({ queryKey: ['dashboard'] })
  })

  it('invalidates specific contact and dashboard on crm:contact:risk_changed', async () => {
    const envelope = { type: 'crm:contact:risk_changed', payload: { contactId: 'c1', emrPatientId: 'p1', riskLevel: 'high', previousRiskLevel: 'low' }, id: 'e3', occurredAt: '', correlationId: '', sourceService: 'crm-service' }
    ...
    expect(mockInvalidateQueries).toHaveBeenCalledWith({ queryKey: ['contact', 'c1'] })
    expect(mockInvalidateQueries).toHaveBeenCalledWith({ queryKey: ['dashboard'] })
  })

  it('invalidates care-gaps and specific contact on crm:care_gap:opened', async () => {
    const envelope = { type: 'crm:care_gap:opened', payload: { careGapId: 'g1', contactId: 'c2', title: 'HbA1c monitoring overdue' }, id: 'e4', occurredAt: '', correlationId: '', sourceService: 'crm-service' }
    ...
    expect(mockInvalidateQueries).toHaveBeenCalledWith({ queryKey: ['contact', 'c2'] })
  })

  it('invalidates contacts on admin:member:created', async () => {
    const envelope = { type: 'admin:member:created', payload: { memberId: 'm1', memberNumber: 'MEM-001', firstName: 'Alice', lastName: 'Smith' }, id: 'e5', occurredAt: '', correlationId: '', sourceService: 'admin-service' }
